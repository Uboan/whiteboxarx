"""Script to generate the implicit (unencoded) and explicit affine layers of a LEA instance for a fixed key."""
...
# LEAInstance = namedtuple('LEAInstance','name,default_rounds,Shrift[8],ws')
...
def get_round_keys(master_key):#128bit Master_key input as a tuple of 4 elements
    default_rounds = LEA.default_rounds
    n = LEA.ws
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(prog="sage -python LEA.py", description="Generate the implicit (unencoded) and explicit affine layers of a LEA instance for a fixed key")
    parser.add_argument("--key", nargs="+", help="the master key given as a hexadecimal representation of the words")
    parser.add_argument("--block-size", nargs="?", type=int, choices=[8, 32, 64, 128], help="the block size in bits of the LEA instance")
